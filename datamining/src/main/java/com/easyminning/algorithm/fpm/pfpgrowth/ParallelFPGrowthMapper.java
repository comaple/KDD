...
public class ParallelFPGrowthMapper extends Mapper<LongWritable,Text,IntWritable, com.easyminning.algorithm.fpm.pfpgrowth.TransactionTree> {
  ...
  protected void map(LongWritable offset, Text input, Context context)
    ...
    for (int j = itemArr.size() - 1; j >= 0; j--) {
      ...
      int groupID = com.easyminning.algorithm.fpm.pfpgrowth.PFPGrowth.getGroup(item, maxPerGroup);
      ...
      if (!groups.contains(groupID)) {
        ...
        context.write(wGroupID, new com.easyminning.algorithm.fpm.pfpgrowth.TransactionTree(tempItems, 1L));
      }
      groups.add(groupID);
    }
  ...
  protected void setup(Context context) throws IOException, InterruptedException {
    ...
    Parameters params = 
      new Parameters(context.getConfiguration().get(com.easyminning.algorithm.fpm.pfpgrowth.PFPGrowth.PFP_PARAMETERS, ""));

    splitter = Pattern.compile(params.get(com.easyminning.algorithm.fpm.pfpgrowth.PFPGrowth.SPLIT_PATTERN,
                                          com.easyminning.algorithm.fpm.pfpgrowth.PFPGrowth.SPLITTER.toString()));
